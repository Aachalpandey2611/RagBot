...
export default function ChatInterface({ auth, setAuth }) {
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        {
          _id: Date.now() + 1,
          role: "assistant",
          content: "Connection error while processing query.",
          sources: [],
        },
      ]);
    } finally {
  ...
  const handleFeedback = async (msgId, type) => {
    try {
      ...
      setMessages((prev) =>
        prev.map((m) => (m._id === msgId ? { ...m, feedback: type } : m)),
      );
    } catch (err) {
  ...
  return (
    <div className="flex h-screen w-full relative">
      ...
      <motion.div
        initial={{ x: -300 }}
        animate={{ x: 0 }}
        className="w-72 glass m-4 flex flex-col hidden md:flex neo-panel"
      >
        ...
        <div className="flex-1 p-4 overflow-y-auto">
          ...
          <motion.div
            ...
            className="bg-white/5 p-3 rounded-lg mb-6 perspective-card"
          >
            ...
            <div className="mb-6">
              ...
              <label className="flex items-center justify-center gap-2 w-full py-3 bg-white/5 hover:bg-white/10 border border-white/10 rounded-lg cursor-pointer transition-all">
                ...
                <input
                  ...
                  accept=".pdf,.txt,.md,.markdown,.json,.csv,.xlsx,.xls"
                />
              </label>
              <div
                ...
                className={`mt-3 rounded-xl border border-dashed p-4 text-center text-xs cursor-pointer transition-all ${
                  dragActive
                    ? "border-primary-400 bg-primary-500/10 text-primary-200 shadow-[0_0_35px_rgba(59,130,246,0.35)]"
      ...
      <div className="flex-1 flex flex-col p-4 pl-0 relative z-10 w-full">
        <div
          ...
          onDragLeave={handleDragLeave}
          onDrop={handleDrop}
          className={`glass flex-1 flex flex-col overflow-hidden h-full relative neo-panel ${dragActive ? "ring-2 ring-primary-500/60" : ""}`}
        >
          {/* Header Mobile */}
          ...
          <div className="flex-1 overflow-y-auto p-4 md:p-8 flex flex-col gap-6">
              ...
              <div className="flex-1 flex flex-col items-center justify-center text-gray-400">
                ...
                {isAdmin && (
                  <p className="text-xs mt-3 opacity-70">
                    Tip: drag and drop your .md files directly into this panel.
                  </p>
                )}
              </div>
            )}
            ...
            <AnimatePresence>
              {messages.map((msg, idx) => (
                ...
                >
                  <div
                    className={`max-w-[85%] md:max-w-[70%] p-4 rounded-2xl message-card ${
                      msg.role === "user"
                  ...
                  >
                    {msg.role === "assistant" && (
                      <div className="text-xs text-primary-400 font-semibold mb-2 flex items-center gap-2">
                        AI Assistant
                      </div>
                    )}
                        ...
                        <div className="mt-4 pt-3 border-t border-white/10">
                          ...
                          <div className="flex flex-wrap gap-2">
                            {msg.sources.map((s, i) => (
                              <span
                                key={i}
                                className="text-xs bg-dark-800/80 px-2 py-1 rounded-md text-gray-300 border border-white/5 cursor-default hover:border-white/20 transition-all select-all"
                              >
                                {s.source}{" "}
                                {s.page !== "N/A" && `(Page ${s.page})`}
                              </span>
                            ))}
                          </div>
                        </div>
                    ...
                    {msg.role === "assistant" && msg._id && (
                      <div className="mt-3 flex items-center justify-end gap-2">
                        <button
                          ...
                          className={`p-1 rounded hover:bg-white/10 transition-colors ${msg.feedback === "like" ? "text-green-400" : "text-gray-500"}`}
                        >
                          <ThumbsUp className="w-4 h-4" />
                        </button>
                        <button
                          onClick={() => handleFeedback(msg._id, "dislike")}
                          className={`p-1 rounded hover:bg-white/10 transition-colors ${msg.feedback === "dislike" ? "text-red-400" : "text-gray-500"}`}
                        >
